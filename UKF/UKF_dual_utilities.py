'''
UKF utility functions for dual estimation implementation
'''
...
from UKF_utilities import unscented_filter_predict as UF_state_predict, \
    unscented_filter_correct as UF_state_correct, \
    Moments, moments2points, points2moments
from UKF_P_utilities import unscented_filter_predict_P as UF_params_predict, \
...
def additive_unscented_filter_dual(w_mu_0, w_sigma_0, x_mu_0, x_sigma_0,
    ...
    '''UKF w/ additive (zero-mean) parameters, transition & obs noises
    ...
    for t in range(T):
        # SP for P(w_{t-1} | y_{0:t-1}) & SP for P(x_{t-1} | y_{0:t-1})
        if t == 0:
            w_mu, w_sigma = w_mu_0, w_sigma_0
            ...
            buffer_noises = Noises(buffer_noise, None)
                
            x_mu, x_sigma = x_mu_0, x_sigma_0
            
        else:
            w_mu, w_sigma = w_mu_filt[t - 1], w_sigma_filt[t - 1]
            x_mu, x_sigma = x_mu_filt[t - 1], x_sigma_filt[t - 1]
        ...
        (w_points_pred, w_moments_pred) = (
            UF_params_predict(
                w_moments_state, noises=Noises(noise, None),
        ...
        w_mu_filt[t], w_sigma_filt[t] = w_moments_filt
        
        '''once t parameters estimated, estimate t state'''
        points_state = moments2points(Moments(x_mu, x_sigma))
        ...
        observation_function = _last_dims(g, t, ndims=1)[0]
        x_mu_filt[t], x_sigma_filt[t] = (
            UF_state_correct(
                observation_function, x_moments_pred, x_points_pred,
                Y[t], sigma_observation=R, params=w_mu_filt[t]
                )
            )
        
        return (x_mu_filt, x_sigma_filt, w_mu_filt, w_sigma_filt)